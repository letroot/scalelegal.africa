<template>
  <div
    class="bg-footer-bg px-10 sm:px-20 lg:px-36 xl:px-48 py-10 sm:py-14 mt-20 inter-font font-light"
  >
    <div class="xl:max-w-screen-2xl xl:mx-auto">
      <div class="flex flex-wrap text-white text-sm">
        <div class="w-1/2 lg:w-3/12 lg:order-1">
          <a
            href="https://scalelegal.africa/about"
            class="block h-7 hover:font-semibold"
            >About</a
          >
          <a
            href="https://scalelegal.africa/services"
            class="block h-7 hover:font-semibold"
            >Our Services</a
          >
          <a
            href="https://scalelegal.africa/expertise"
            class="block h-7 hover:font-semibold"
            >Expertise</a
          >
          <a
            href="https://scalelegal.africa/team"
            class="block h-7 hover:font-semibold"
            >The Team</a
          >
          <a
            href="https://blog.scalelegal.africa/"
            class="block h-7 hover:font-semibold"
            >Blog</a
          >
        </div>

        <div class="w-1/2 lg:w-3/12 lg:order-3 flex justify-between">
          <div>
            <a href="#" class="block h-7 hover:font-semibold">Facebook</a>
            <a href="#" class="block h-7 hover:font-semibold">LinkedIn</a>
            <a href="#" class="block h-7 hover:font-semibold">Instagram</a>
            <a href="#" class="block h-7 hover:font-semibold">Twitter</a>
          </div>
          <div>
            <a href="#" @click="scrollToTop" class="block">↑</a>
          </div>
        </div>

        <div class="w-1/2 lg:w-6/12 lg:order-2 mt-10 lg:mt-0">
          <p class="h-12 sm:h-7">
            Phone: <a href="#" class="hover:font-semibold">+234 808 906 2718</a>
          </p>
          <p class="h-12 sm:h-7">
            Email:
            <a href="#" class="hover:font-semibold">info@scalelegal.africa</a>
          </p>
        </div>
      </div>
      <div class="text-built-with text-sm mt-10">
        <p>
          Built with ❤ by
          <a href="#" class="hover:font-semibold">Cheapmedia Nigeria</a>
        </p>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  methods: {
    scrollToTop() {
      window.scrollTo(0, 0);
    },
  },
};
</script>

<style>
@import url("https://fonts.googleapis.com/css2?family=Archivo:ital,wght@0,400;0,500;0,600;0,700;1,400;1,500;1,600;1,700&family=Inter:wght@100;200;300;400;500;600;700&display=swap");

.inter-font {
  font-family: "Inter", sans-serif;
}
</style>
